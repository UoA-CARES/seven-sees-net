...
def train_one_epoch(epoch_index, interval=5):
    ...
    for i, (_, _, _, _, right_hand, _, _, _, targets)  in enumerate(train_loader):
        right_hand, targets = right_hand.to(device), targets.to(device)
...
def validate():
    ...
    with torch.inference_mode():
        for i, (_, _, _, _, right_hand, _, _, _, vtargets)  in enumerate(test_loader):
            right_hand, vtargets = right_hand.to(device), vtargets.to(device)

            vtargets = vtargets.reshape(-1, )

            voutputs = model(right_hand)
...
if __name__ == '__main__':
    ...
    wandb.init(entity="cares", project="seven-sees",
            group="right-hand-only")

    # Set up device agnostic code
    device= 'cuda'

    # Configs
    work_dir = 'work_dirs/seven-seas-v1-right-hand/'
    batch_size = 256
